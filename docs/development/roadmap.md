...
- [ ] **(later)** GNU **longname** for names > 255 B; per-image `platform` passthrough in index descriptors (single-node is always `amd64/linux`); empty-registry `ref.name` cosmetic.
