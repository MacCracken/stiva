...
- [ ] **(v3.0.3 follow-up)** **`docker-archive` read path** (`docker save` tarball → OCI: `manifest.json` + config + uncompressed layer tars → gzip layers + OCI manifest). Detected + reported as unsupported by `load` today. Plus the A2-review external-ref hardening: `_il_parse_full_ref` for a port-registry `ref.name` without a tag / an empty-registry `ref.name`, and carrying each descriptor's real `platform` through the index rewrite (all unreachable for stiva-written `local/<name>:<tag>` layouts).
