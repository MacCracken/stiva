...
**2186 tests** across `tests/*.tcyr`: `stiva.tcyr` (667), `registry.tcyr` (421),
...
| `tests/runpath.tcyr` (358) | the synchronous **run path** + image store: `generate_spec`, `build_sandbox` (backend cascade / min-score), `exec_container`, `exec_in_container`, `send_signal`, cgroup resolve/quota/limits, security scoring, image store round-trips (real `import`→`index.json`→reconstruct→`remove`/`gc`/`tag`), `scan_output` over kavach's externalization gate, and `container_manager_diff` |
