...
## Decision
Default backend selection uses a conservative OCI → Process fallback chain. Strength-based selection via `min_isolation_score` is opt-in via `ContainerConfig`:
