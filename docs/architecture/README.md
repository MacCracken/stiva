...
| # | Item | In one line |
|---|------|-------------|
| 001 | [Typed struct field access can silently read garbage (cycc)](001-cycc-struct-id-miscompile.md) | The struct-id ↔ SIMD-sentinel miscompile is live at 6.4.78; it is per-function *and* per-compilation-unit, and it fails silently more often than it crashes. Explains every raw-offset accessor in the codebase |
