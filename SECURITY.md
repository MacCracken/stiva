...
- All container processes run through a kavach sandbox with seccomp (`basic` profile) + NO_NEW_PRIVS. ⛔ **Landlock is not currently applied** — stiva builds from `policy_basic()`, which carries no Landlock rules, and no code path calls `policy_landlock_add`. Filesystem confinement comes from the rootfs chroot and mount namespace.
...
- Build cache keyed by a **metadata** fingerprint (path, mode, uid, gid, size, ns-mtime) over the copy source — **not** content. An edit preserving all of those will not invalidate it; `rm -rf {root}/cache` forces a rebuild.
...
- ID-mapped mounts (`X-mount.idmap=`) are emitted into the runtime spec for rootless volumes, but ⛔ `RuntimeSpec.mounts` is write-only — **no bind mount is performed at all** (roadmap v3.1.0 item 9).
